...
### Fixed
- **CSS migration preserves stable Etch style IDs end-to-end (2026-03-08):** The converter now derives deterministic IDs for migrated Bricks classes, the style importer preserves incoming style keys instead of reindexing them, and existing selector-keyed target data is repaired on re-import. This keeps `efs_style_map`, `etch_styles`, and block `styles[]` references aligned so classes remain visible in the Etch builder and on the frontend.
- **Receiving progress now uses real migration totals instead of inflated placeholders (2026-03-08):** CSS totals are now calculated from the actual referenced-class payload, target-side total reconciliation no longer pins stale `items_total` values, and zero-media runs no longer pre-seed media totals. This fixes migrations that showed `CSS` and `Pages` at 100% while the overall receiving progress stayed near 0%.
- **Batch post completion now counts the correct source post types (2026-03-08):** Post batches now carry `source_post_type`, successful imports are recorded consistently, and receiving-state aggregation increments the original source buckets. This fixes `complete_migration()` failures where mapped types such as `page -> post` never reached their expected per-type totals.
- **Completion reconciliation now accounts for failed or unsendable media files (2026-03-08):** Source-side media totals now exclude missing files, failed media counts are forwarded during finalization, and target-side completion reconciles expected receipts before enforcing totals. This fixes `Migration not yet complete` errors caused by source attachments that existed in the database but not on disk.
- **WordPress unit bootstrap no longer emits duplicate memory-constant warnings (2026-03-08):** The test bootstrap now suppresses the known `WP_MEMORY_LIMIT` and `WP_MAX_MEMORY_LIMIT` redefinition warnings from the Docker test harness without masking unrelated notices.

### Added
- **Regression coverage for CSS/style-map integrity and completion accounting (2026-03-08):** Added targeted WordPress unit tests for CSS import stability, deterministic CSS conversion IDs, CSS payload totals, batch post completion, checkpoint validation, and media-file filtering/reconciliation.
- **Bilingual CSS migration runbooks for Bricks to Etch (2026-03-08):** Added operational guides in `docs/css-migration-bricks-to-etch-de.md` and `docs/css-migration-bricks-to-etch-en.md`, plus an updated `docs/css-converter-architecture.md` that documents the current converter/importer architecture and failure modes.

### Fixed
- **Autoload consistency: all `Bricks2Etch\\Core\\*` classes now live under `includes/core/` with no fallback (2026-03-06):** Moved `EFS_DB_Installer`, `EFS_Error_Handler`, `EFS_Migration_Manager`, `EFS_Migration_Token_Manager`, and `EFS_Plugin_Detector` off the `includes/` root into `includes/core/`, removed the manual autoloader's `Core\\` root fallback, and updated bootstrap, scripts, tests, and documentation to the canonical paths. Composer still uses `classmap` for other legacy namespaces, but `Core\\` is now strict.
- **Uninstall cleanup: 19 missing options + wp_efs_settings table (2026-03-06):** `uninstall()` left behind 19+ option keys (`efs_current_migration_id`, `efs_active_migration`, `efs_receiving_migration`, `efs_migration_checkpoint`, `efs_last_migration`, `efs_dismissed_migration_runs`, `efs_post_mappings`, `efs_media_mappings`, `efs_acss_inline_style_map`, `efs_migration_log`, `efs_style_map`, `efs_registered_cpts`, `efs_component_map`, `efs_migration_steps`, `efs_migration_stats`) plus all dynamic per-migration options (`efs_progress_data_*`, `efs_steps_data_*`, `efs_stats_data_*`, `efs_inline_css_*`, `efs_inline_js_*`). The `wp_efs_settings` table was also never dropped. Replaced the growing hardcoded list with a single `DELETE … WHERE option_name LIKE 'efs\_%'` that covers all current and future options. Added `DROP TABLE IF EXISTS wp_efs_settings`. Action Scheduler DELETE changed to a prefix LIKE (was a contains LIKE — could theoretically match foreign hooks).
- **Security: JWT token options no longer autoloaded (2026-03-06):** `save_token_value()`, `save_token_data()`, and `OPTION_TOKEN_EXPIRES` all called `update_option()` without `$autoload = false`. The full JWT (valid 8h auth credential) was included in WordPress's `alloptions` query on every page load — unnecessary exposure and memory waste. Fixed by adding `false` as the autoload argument to all three calls.
- **Security: Rate limiter now enforced in `verify_request()` (2026-03-06):** `EFS_Rate_Limiter` was instantiated and injected into every `EFS_Base_Ajax_Handler` but never called. All AJAX endpoints were effectively unrate-limited. Fixed by adding a `check_rate_limit('user_X', 'efs_ajax', 60, 60)` call inside `verify_request()` — 60 req/min per authenticated user, 429 response on exceed. Applies to all 12 handlers automatically via the base class.
- **Security: JWT signing secret rotated on token revoke (2026-03-06):** `revoke_current_migration_key()` deleted the token from wp_options but left `efs_migration_jwt_secret` intact. If an attacker previously copied the secret from the database, they could forge new valid JWTs after revocation. Fixed by generating and storing a new 32-byte random secret during every revoke, rendering all previously issued tokens cryptographically invalid.
- **Security: `efs_headless_job_count` added to uninstall cleanup (2026-03-06):** The new diagnostic option was missing from the uninstall cleanup list in `includes/core/EFS_DB_Installer.php`.
- **Missing Utils\\ namespace in autoloader (2026-03-06):** `Bricks2Etch\Utils\Query_Paginator` was referenced in `css_converter.php` and `media_migrator.php` but `Utils\\` had no entry in `includes/autoloader.php` namespace_map, causing a fatal "Class not found" error during every migration. Added `'Utils\\' => 'utils/'` to the map.
- **PermissionCallbacksTest fatal error (2026-03-06):** `setUpBeforeClass($factory)` signature was incompatible with `Yoast\PHPUnitPolyfills\TestCases\TestCase::setUpBeforeClass(): void`, causing a PHP Fatal Error that killed the entire CI test run with exit code 255. Renamed to `wpSetUpBeforeClass($factory)` (the correct WordPress-native hook) and removed the `parent::` call — `WP_UnitTestCase::setUpBeforeClass()` calls `wpSetUpBeforeClass` internally.
- **optimize-json-logging: gutenberg_generator.php (2026-03-06):** `wp_json_encode($element['settings'])` was evaluated as a function argument even when `WP_DEBUG` is false, wasting CPU on JSON encoding that `log_info()` would discard. Fixed by passing `$element['settings']` as the separate `$data` argument to `log_info()`, so encoding only happens inside the `WP_DEBUG` gate.
- **add-cloud-ip-whitelist: loopback runner (2026-03-06):** `169.254.169.254` (AWS/GCP instance metadata endpoint) added to the `$is_localhost` whitelist in `class-action-scheduler-loopback-runner.php`. Prevents loopback requests from being rejected on cloud deployments where the container's outbound IP is the link-local metadata address.
- **fix-headless-job-count-atomic: progress manager (2026-03-06):** `increment_headless_job_count()` used a Read-Modify-Write pattern that allowed parallel headless jobs to produce stale counter values, potentially suppressing the > 50 warning. Replaced with an atomic `UPDATE … SET option_value = option_value + 1` SQL statement on a dedicated `efs_headless_job_count` option. Falls back to `add_option(..., 1)` on first call. Object cache invalidated via `wp_cache_delete` before returning the new value.

- **10f Shutdown-handler closure (2026-03-05):** `EFS_Batch_Processor::release_lock_on_shutdown()` (static class method) replaced with an inline closure that captures `$lock_uuid` and `$migration_id` by value via `use`. Removed the `$shutdown_lock_key` static property entirely. The closure has no class-reference dependency, so it executes safely even when a PHP fatal error unloads the class during shutdown. The `finally` block in `process_batch()` still releases the lock on the normal code path; the closure is effectively a no-op in that case because `lock_uuid` is already `NULL` and the `WHERE lock_uuid = %s` condition matches zero rows.
- **Item 5 Nonce verification verified complete (2026-03-05):** Audit confirmed all AJAX handlers call `verify_request('manage_options')` (which calls `check_ajax_referer()`). The three legacy handlers in `api_endpoints.php` call `check_ajax_referer()` directly. REST endpoints correctly use WP-native permission callbacks (no explicit nonce needed for REST). Previous "partial" assessment in optimierungen.md was outdated — no code changes needed.
- **10j DB transaction for checkpoint + progress (2026-03-05):** `EFS_DB_Installer::save_checkpoint_and_progress()` wraps two `UPDATE` statements on `wp_efs_migrations` (checkpoint_data write + processed_items/status write) in a `START TRANSACTION / COMMIT` block. If the checkpoint write conflicts on version (another process wrote first) the transaction is rolled back and the caller falls back to the standard individual writes. `EFS_WordPress_Migration_Repository::save_batch_state()` orchestrates the lookup of the current checkpoint_version, the transactional call, and the wp_options fallback. `EFS_Batch_Phase_Runner::run_phase()` calls `save_batch_state()` via `method_exists()` guard (preserves backward compat with non-transactional implementations) instead of the previous separate `save_checkpoint()` call. Scope: only `wp_efs_migrations` (WordPress tables cannot be reliably transacted).
- **10i Idempotency guard in batch phase runner (2026-03-05):** After a successful HTTP send, the post/media ID is now added to `$processed_ids_set`, which is persisted in the checkpoint. On every batch start, the current batch is filtered against this set before any HTTP calls are made. This ensures that if an HTTP response is lost (network timeout/crash) the item is skipped on retry, preventing duplicate posts/media on the target site.
- **10h Atomic progress heartbeat implemented (2026-03-05):** `EFS_DB_Installer::touch_progress_heartbeat()` now performs a single direct `UPDATE … SET updated_at = %s WHERE migration_uid = %s AND status = 'in_progress'` with no read-modify-write cycle. Eliminates the race condition where two concurrent heartbeat writes could overwrite each other. Called via `EFS_DB_Migration_Persistence::touch_progress_heartbeat()` → `EFS_Progress_Manager::touch_progress_heartbeat()`.
- **DB schema v1.2.0 — checkpoint_data, checkpoint_version, lock_uuid, locked_at columns (2026-03-05):** `wp_efs_migrations` gains four new columns needed for optimistic locking and DB-backed checkpoint storage. Fresh installs receive them via `CREATE TABLE`; existing installs receive them via `maybe_add_columns()` (idempotent `SHOW COLUMNS` + `ALTER TABLE`) which runs inside the upgraded `install()` on first load after version bump. `maybe_upgrade_db()` (priority-0 `plugins_loaded` hook) triggers this automatically — no manual reactivation needed.
- **EFS_DB_Installer: three previously-undefined static methods added (2026-03-05):** `touch_progress_heartbeat()`, `save_checkpoint_atomic()`, and `get_checkpoint_with_version()` were referenced by `EFS_DB_Migration_Persistence` and `EFS_WordPress_Migration_Repository` but did not exist, causing fatal "Call to undefined method" errors in production whenever a migration was running and the migration ID was set. All three are now fully implemented with proper SQL (`prepare()`).
