...
class EFS_API_Client {
	...
	private function send_request( $url, $jwt_token, $endpoint, $method = 'GET', $data = null, $api_base = 'efs/v1' ) {
		...
		$args = array(
			'method'  => $method,
			'timeout' => (int) apply_filters( 'efs_api_request_timeout', 60 ), // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- hook is correctly prefixed with efs_
			'headers' => array(
	...
	/**
	 * Send Bricks global CSS to the target site's etch_global_stylesheets option.
	 *
	 * @param  string $url       Target base URL.
	 * @param  string $jwt_token Migration token.
	 * @param  array  $payload   Stylesheet entry: {id, name, css, type}.
	 * @return array|\WP_Error
	 */
	public function send_global_css( $url, $jwt_token, $payload ) {
		return $this->send_request( $url, $jwt_token, '/import/global-css', 'POST', $payload );
	}
