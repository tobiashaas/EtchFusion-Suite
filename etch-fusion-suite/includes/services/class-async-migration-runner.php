...
class EFS_Async_Migration_Runner {
	...
	public function run_migration_execution( $migration_id = '' ) {
		...
		try {
			...
			// Migrate Bricks global CSS (customCss) to Etch global stylesheets.
			// Runs in the same CSS phase, right after class migration.
			$this->progress_manager->update_progress( 'css', 75, __( 'Migrating global CSS...', 'etch-fusion-suite' ) );
			$global_css_result = $this->css_service->migrate_global_css( $target, $migration_key );
			if ( is_wp_error( $global_css_result ) ) {
				// Non-fatal: log and continue rather than aborting the whole migration.
				$this->migration_logger->log( $migration_id, 'warning', 'Global CSS migration failed: ' . $global_css_result->get_error_message() );
			} else {
				$skipped = ! empty( $global_css_result['skipped'] );
				$this->migration_logger->log( $migration_id, 'info', $skipped ? 'Global CSS skipped (none found)' : 'Global CSS migrated' );
			}
