<?php
/**
 * Shortcode Element Converter
 *
 * Converts Bricks shortcode elements to etch/raw-html Gutenberg blocks.
 *
 * @package Bricks_Etch_Migration
 * @subpackage Converters\Elements
 * @since 0.5.0
 */

namespace Bricks2Etch\Converters\Elements;

use Bricks2Etch\Converters\EFS_Base_Element;

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

class EFS_Element_Shortcode extends EFS_Base_Element {

	/**
	 * Element type.
	 *
	 * @var string
	 */
	protected $element_type = 'shortcode';

	/**
	 * Convert Bricks shortcode element to etch/raw-html block.
	 *
	 * @param array $element  Bricks element.
	 * @param array $children Child elements (unused for shortcode).
	 * @param array $context  Optional. Conversion context.
	 * @return string Gutenberg block HTML.
	 */
	public function convert( $element, $children = array(), $context = array() ) {
		$settings = $element['settings'] ?? array();

		$shortcode = isset( $settings['shortcode'] ) ? $settings['shortcode'] : '';
		$shortcode = is_string( $shortcode ) ? $shortcode : '';

		$label = $this->get_label( $element );

		$attrs = array(
			'content'  => $shortcode,
			'unsafe'   => false,
			'metadata' => array(
				'name' => $label,
			),
		);

		$attrs_json = wp_json_encode( $attrs, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES );

		return '<!-- wp:etch/raw-html ' . $attrs_json . ' -->' . "\n" .
			'<!-- /wp:etch/raw-html -->';
	}
}
