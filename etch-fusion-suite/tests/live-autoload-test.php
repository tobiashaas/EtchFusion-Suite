<?php
/**
 * Live autoload test — runs inside Docker via npm run wp -- eval-file.
 *
 * Tests class_exists()/interface_exists() for all key Bricks2Etch\ classes
 * after the plugin has been loaded by WordPress.
 */
...
$classes = [
    ...
    // Parsers (root-level legacy files)
    ...
    // Vendor (Strauss-prefixed)
    'EtchFusionSuite\\Vendor\\Firebase\\JWT\\JWT',
    'EtchFusionSuite\\Vendor\\Psr\\Container\\ContainerInterface',
    // Action Scheduler globals
    'ActionScheduler',
...
foreach ( $classes as $c ) {
    if ( class_exists( $c ) || interface_exists( $c ) || ( function_exists( 'class_exists' ) && class_exists( $c, false ) ) ) {
        $pass++;
    } else {
        $fail[] = $c;
    }
...
echo 'PASS: ' . $pass . ' / FAIL: ' . count( $fail ) . ' / TOTAL: ' . count( $classes ) . PHP_EOL;
foreach ( $fail as $f ) {
    echo '  MISSING: ' . $f . PHP_EOL;
}
if ( empty( $fail ) ) {
    echo 'All autoloader checks passed.' . PHP_EOL;
}
